...
function flattenDictionary(dict) {
    // your code goes here
    /**
     * for in  index, judge every value if it object
     * if object for in and output prev key+ this key
     */
    var obj = {};
    recursive(dict, obj);
    return obj;
...
function recursive(dict, obj, level) {
    for (let i in dict) {
        let levelKey = level ? (i ? `${level}.${i}` : `${level}`) : i;

        if (dict[i] instanceof Object) {
            recursive(dict[i], obj, levelKey);
        } else {
            obj[levelKey] = dict[i];
        }
    }
